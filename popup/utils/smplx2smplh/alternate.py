...
class MeshInMemory(Dataset):
    ...
    def __getitem__(self, index):
        ...
        return {
            ...
            'indices': index,
            'paths': "{}/{}_{}_{:04d}.obj".format(self.seq_info['sbj_id'],
                                                   self.seq_info['obj_name'],
                                                   self.seq_info['action'],
                                                   index)
        }
...
def create_smplx_meshes(seq_data, cfg):
    ...
    # create SMPL-X mesh
    sbj_mesh = os.path.join(str(cfg.grab_path), seq_data["body"]["vtemp"])  # template subject mesh
...
def run_smplx2smplh_conversion(dataset, seq_info, cfg):
    ...
    for batch_index, batch in enumerate(tqdm.tqdm(dataloader)):
        ...
        if cfg.save_meshes:
            for i, index in enumerate(indexes):
                output_mesh_path = os.path.join(seq_info["output_seq_path"], "{:04d}.obj".format(index))
                mesh_smplh = np_mesh_to_o3d(vertices[i], faces)
                o3d.io.write_triangle_mesh(output_mesh_path, mesh_smplh)
                
                output_mesh_path = os.path.join(seq_info["output_seq_path"], "smplx_{:04d}.obj".format(index))
    ...
    # save converted data
    sequence_data_path = os.path.join(seq_info["output_seq_path"], "sequence_data.pkl")
    seq_info["body"] = body
    with open(sequence_data_path, 'wb') as fp:
...
if __name__ == "__main__":
    ...
    parser.add_argument('-g', "--grab-path", type=str)
    parser.add_argument('-s', "--smplx-path", type=str)
    ...
    _all_seqs = os.listdir(os.path.join(cfg['grab_path'], 'grab'))
    
    all_seqs = []
    if len(cfg['objects']) > 0:
        for seq in _all_seqs:
            if seq.split("_")[0] in cfg['objects']:
                all_seqs.append(seq)
    else:
        all_seqs = _all_seqs
    ...
    for sequence in tqdm.tqdm(all_seqs, total=len(all_seqs), ncols=80):
        ...
        seq_info = {
            ...
            "action": "_".join(sequence.split("_")[1:]),
            "obj_name": str(seq_data["obj_name"]),
        }
        seq_info["output_seq_path"] = \
            os.path.join(cfg['grab_path'], "grab_smplh", "{}/{}_{}".format(seq_info['sbj_id'],
                                                                           seq_info['obj_name'],
                                                                           seq_info['action']))
        os.makedirs(seq_info["output_seq_path"], exist_ok=True)
