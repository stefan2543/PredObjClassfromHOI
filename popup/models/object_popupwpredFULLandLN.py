...
class ObjectPopup(nn.Module):
    ...
    def encode_1st_stage(
        ...
        return enc_sbj_global, pred_center

    def encode_2nd_stage(
        self, enc_subject, subject, obj_classids,
        obj_center=None, obj_scales=None, obj_keypoints=None
    ):
        batch_size, _, _ = subject.shape
        ...
        if self.with_classifier:
            if obj_classids is None:
                enc = torch.cat([enc_subject, enc_sbjobj])
                pred_class = self.classifier(enc)
            else:
                ...
                pred_class = self.classifier(classes_enc)
        else:
            pred_class = None

        return enc_sbjobj, pred_class
    ...
    def encode(
        ...
        enc_subject, pred_center = self.encode_1st_stage(
            subject
        )

        pred_class = self.encode_2nd_stage(enc_subject, 
            subject, pred_center, obj_classids, obj_center=obj_center,
